...
const authenticateRegistrationPageSuccess = (pathname, replace, callback) =>
  callback();
...
export const authenticateRegistrationPage = (pathname, replace, callback) => (
  dispatch,
  getState
) => {
  ...
  return axios
    ...
    .then(res => {
      if (!res.data.apiError) {
        dispatch(
          authenticateRegistrationPageSuccess(pathname, replace, callback)
        );
      } else {
        dispatch(
          authenticateRegistrationPageFail(
            res.data,
            pathname,
            replace,
            callback
    ...
    .catch(error =>
      dispatch(
        authenticateRegistrationPageFail(error, pathname, replace, callback)
      )
    );
};
  ...
  return axios
    ...
    .catch(error => {
      ...
      throw new SubmissionError({
        _error: 'Your message in was unsuccessful. Please try again.',
      });
    });
};
