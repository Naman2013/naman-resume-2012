...
class ReserveConfirm extends Component {
  ...
  cancelMissionSlot() {
    ...
    const { scheduledMissionId, uniqueId } = currentMissionSlot.missionList[0];
  ...
  */
  handleMissionReservationResponse() {
    ...
    const { explanation, missionAvailable, tip } = missionData;
  ...
  renderModalContent() {
    ...
    if (!missionAvailable) {
      return <ReservationError closeModal={closeModal} message={explanation} />;
    }
    ...
    return (
      <div>
        <div className="title-bar">
          ...
          <h3 className="title">
            Please complete your reservation form within{' '}
            <InlineCountdown
              startTime={missionData.expires}
              exitAction={this.cancelMissionAndCloseModal}
            />
          </h3>
        </div>
