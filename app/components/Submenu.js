            ...
            {this.props.data.map((child, i) => {
              if (typeof child.label !== 'undefined') { // FIXME: Is imposible when server return undefined
                switch (child.label.en) {
                  case 'Upcoming Shows': {
                    return <UpcomingComponent key={i} source="https://saturn.slooh.com:444/events/upcoming?limit=50" refreshIntervalDelay={60000} />;
                  }
                  case 'Observatories': {
                    return <ListObservatories key={i} source="https://saturn.slooh.com:444/obs/compact" refreshIntervalDelay={60000} />;
                  }
                  case 'Hot this Month': {
                    return <ListHotThisMonth key={i} source="https://saturn.slooh.com:444/hot/thisMonth" />;
                  }
                  default: {
